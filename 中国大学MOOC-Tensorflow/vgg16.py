...
class Vgg16():
    ...
    def fc_layer(self,x,name): 
        with tf.Variable_scope(name):#根据命名空间做全连接层的计算
    ...
    def forward(self,images):
	    ...
	    #从RGB转换色彩通道到RGB,也可以使用cv中的RGBtoBGR
        red,green,blue=tf.split(rgb_scaled,3,3)
        assert red.get_Shape().as_list()[1:]==[224,224,1]
        assert green.get_shape().as_list()[1:]==[224,224,1]
        assert blue.get_shape().as_list()[1:]==[224,224,1]
        #以上assert都是加入断言,用来判断每个操作的维度变化是否和预期一致
        bgr=tf.concat([blue-VGG_MEAN[0],green-VGG_MEAN[1],red-VGG_MEAN[2]],3) 
        #逐样本减去每个通道的像素平均值,这种操作可以移除图像的平均亮度值
        #该方法长用在灰度图像上
        assert bgr.get_shape().as_list()[1:]==[224,224,3]
        #接下来构建VGG的16层网络(包含5段卷积,3层全连接层)
        #并逐层根据命名空间读取网络参数
        
        #第一层卷积,含有两个卷积层,后面接最大池化层,用来所辖图片尺寸
        self.conv1_1=self.conv_layer(bgr,"conv1_1")
        #传入命名空间的name,来获取该层的卷积核和偏置,并做卷积运算
        #最后返回经过激活函数后的值
        self.conv1_2=self.conv_layer(self.conv1_1,"conv1_2")
        self.pool1=self.max_pool_2x2(self.conv1_2,"pool1")
        
        #第二段卷积层,同样包含两个卷积层,一个最大池化层
        self.conv2_1=self.conv_layer(self.pool1,"conv2_1") 
        self.conv2_2=self.conv_layer(self.conv2_1,"conv2_2")
        self.pool2=self.max_pool_2x2(self.conv2_2,"pool2")
		
        #第三段卷积层,同样包含三个卷积层,一个最大池化层
        self.conv3_1=self.conv_layer(self.pool2,"conv3_1") 
        self.conv3_2=self.conv_layer(self.conv3_1,"conv3_2")
        self.conv3_3=self.conv_layer(self.conv3_2,"conv3_3")
        self.pool3=self.max_pool_2x2(self.conv3_3,"pool3")

        #第四段卷积层,同样包含三个卷积层,一个最大池化层
        self.conv4_1=self.conv_layer(self.pool3,"conv4_1") 
        self.conv4_2=self.conv_layer(self.conv4_1,"conv4_2")
        self.conv4_3=self.conv_layer(self.conv4_2,"conv4_3")
        self.pool4=self.max_pool_2x2(self.conv4_3,"pool4")	
        
        #第五段卷积层,同样包含三个卷积层,一个最大池化层
        ...
        assert self.fc6.get_shape().as_list()[1:]==[4096] #4096是该输出后的长度
        ...
        #经过最后一层的全连接后,在做softmax分类,得到属于各类别的概率
